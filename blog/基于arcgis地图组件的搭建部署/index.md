...
|天地图|支持离线访问、是政府专用地图|UI不够美观、文档不够清楚|
|arcgis|支持离线访问、UI上可做到切换不同图层|文档不够清楚

最终由于是政府相关的项目，需要支持离线访问，同时那边所采用的地图组件也是 arcgis，所以最终采用 arcgis

# 前期准备

## 三方库本地化

考虑到在政府内网离线运行，所以要对 arcgis 的 js 库进行本地化，大致经历了以下过程：
...
写出一个基本的 demo，在外网的情况下以天地图的图层进行访问，在政府内网以内部服务器返回的图层进行访问，在内网验证成功，未发现多余请求，主要验证代码如下：
...
1. 政府内网接口返回的经纬度单位是转换墨卡托投影单位，需要用 gcoord 三方库做转换
2. get 参数处理
3. 增加成功、失败总数统计
...
经过以上 2 轮修复，还是有部分数据未查到结果，最终采用政府内部（另外合作商）经纬度去修复
