...
## 广州探迹科技有限公司——中级前端工程师（2018 年 6 月 ~ 至今）

- 集客、智能呼叫前端主程序员
- 负责项目搭建、公用组件封装、架构优化、新人培训、技术分享、基础工具开发、日常迭代开发
...
- 任务
  - 负责集客产品脚手架搭建，编译时间 `10s` 左右，首屏时间 `439ms`，首屏加载大小 `1.1MB`，具体搭建过程见 [Webpack脚手架搭建笔记](https://blog.towavephone.com/webpack-template-new-project/)
  - 负责集客顾客端脚手架搭建，需要兼容桌面端、移动端、微信公众号以及小程序，使用 webpack 生成桌面端、移动端代码，搭建过程见 [记一次组件打包为链接的实践](https://blog.towavephone.com/components-pack-as-library/)。之后由于加载速度较慢，采用轻量级三方库 react-lite，所有组件使用 antd 底层框架，编译时间 `10s` 左右，使用懒加载、预加载技术，将首屏时间减小到 `236ms`，首屏加载大小减小到 `101KB`，具体搭建过程见 [构建多平台轻量化组件的实践](https://blog.towavephone.com/building-platform-lightweight-components/)。其中也踩了一些移动端的坑，踩坑过程见 [移动端适配汇总](https://blog.towavephone.com/mobile-adaptation-summary/)
  - 负责集客表单脚手架的搭建，使用 gulp + nodejs 搭建，原生 javascript 实现集客表单，首屏时间 `264ms`，加载大小 `85.9KB`，具体搭建过程见 [轻量级网站构建实践](https://blog.towavephone.com/lightweight-website-construction/)
- 成果
  - 集客、集客顾客端、集客表单脚手架编译时间普遍较快提升了开发体验，产品在各端加载很快，大大提升用户体验
